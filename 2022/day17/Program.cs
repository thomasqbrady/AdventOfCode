        ...
        static void PartOne(string input)
        {
            ...
            for (int i = 0;i < 4;i++) {
                ...
                while (fallingShape != null) {
                    ...
                    if (board.ContainsKey((nextX, fallingShape.Y)) || board.ContainsKey((nextX + fallingShape.Width, fallingShape.Y)) || nextX < 0 || nextX + fallingShape.Width > chamberWidth) {
                    ...
                    for (int row = 0;row < shapeStrings.Length;row++) {
                        for (int col = 0;col < shapeStrings[row].Length;col++) {
                            if (shapeStrings[row][col] == '#') {
                                ...
                                Console.WriteLine($"Checking to see if there is something for it to collide with at {fallingShape.X + col}:{nextY - row}");
                                (int, int) placeToCheck = (fallingShape.X + col, nextY - row);
