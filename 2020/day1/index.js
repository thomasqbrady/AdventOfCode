...
function findIt() {
  for (let i = 0;i < count;i++) {
    for (let j = 0;j < count;j++) {
      let numbers = [expenses[i], expenses[j]];
      let lower = Math.min(...numbers);
      let higher = Math.max(...numbers);
      if (i != j && cache['' + lower + '+' + higher] !== 1) {
        if (lower + higher == 2020) {
          return lower * higher;
        }
        cache['' + lower + '+' + higher] = 1;
      }
    }
  }
}
  ...
  for (let i = 0;i < count;i++) {
    for (let j = 0;j < count;j++) {
      for (let k = 0;k < count;k++) {
        if (i != j && i != k && j != k) {
          let numbers = [expenses[i], expenses[j], expenses[k]];
