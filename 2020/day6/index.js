...
function sumGroups() {
  let responses = input.split('\n\n');
  let sum = 0;
  responses.forEach((response) => {
    let group = response.trim().split('\n');
    let groupResponses = {};
    group.forEach((member) => {
      let questions = member.split('');
      questions.forEach((q) => {
        if (!groupResponses[q]) {
          groupResponses[q] = 1;
        } else {
    ...
    let answers = Object.keys(groupResponses);
    sum += answers.length;
  });
  console.log(sum);
}
...
function sumGroups2() {
  ...
  responses.forEach((response) => {
    ...
    answers.forEach((answer) => {
      if (group.length == groupResponses[answer]) {
        sum++;
      }
    });
  });
