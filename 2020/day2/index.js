let input = `13-17 s: ssssssssssssgsssj
7-9 p: pnlzhcppvl
5-6 z: zzbwrv
...
function challengeTwo() {
  ...
  input.split('\n').forEach((entry) => {
    ...
    let letter = parts[1];
    let password = parts[2];
    let positions = counts.split('-');
    let first = positions[0] - 1;
    let second = positions[1] - 1;
    letter = letter.substr(0,1);
    let letterCount = 0;
    ...
    console.log(validPasswords + ' of ' + passwordCount + ' are valid');
  });
}