...
input = `LLLLLL.LL.LL.LLLLLL.LLL.L.LLLLLLLLLLLLLLLL.LLLLLLLLLLL.LLLL.LLLLLLL.LLLLLLLL.LLLLLLLLLLLLLLLLLLLL
...
function checkSurroundingSeats(seats, seatRow, seatCol) {
  let emptySeats = 0;
  let occupiedSeats = 0;
  for (let checkRow = seatRow - 1;checkRow <= seatRow + 1;checkRow++) {
    if (checkRow > -1 && checkRow < seats.length) {
      for (let checkCol = seatCol - 1;checkCol <= seatCol + 1;checkCol++) {
        if (checkCol > -1 && checkCol < colCount && (checkRow != seatRow || checkCol != seatCol)) {
          let position = seats[checkRow][checkCol];
          switch (position) {
            case 'L':
              emptySeats++;
              break;
...
function checkPath(seats, run, rise, startRow, startCol) {
  let nextVisibleSeatRow = startRow + rise;
  let foundOccupiedSeat = false;
  if (nextVisibleSeatRow > -1 && nextVisibleSeatRow < seats.length) {
    let nextVisibleSeatCol = startCol + run;
    if (nextVisibleSeatCol > -1 && nextVisibleSeatCol < colCount) {
      let nextVisibleSeat = seats[nextVisibleSeatRow][nextVisibleSeatCol];
      switch (nextVisibleSeat) {
        case 'L':
          return false;
        case '.':
          return checkPath(seats, run, rise, nextVisibleSeatRow, nextVisibleSeatCol);
          break;
        case '#':
          return true;
      }
...
function checkSurroundinSeatsAdvanced(seats, seatRow, seatCol) {
  let n = checkPath(seats, 0, -1, seatRow, seatCol);
  let ne = checkPath(seats, 1, -1, seatRow, seatCol);
  let e = checkPath(seats, 1, 0, seatRow, seatCol);
  let se = checkPath(seats, 1, 1, seatRow, seatCol);
  let s = checkPath(seats, 0, 1, seatRow, seatCol);
  let sw = checkPath(seats, -1, 1, seatRow, seatCol);
  let w = checkPath(seats, -1, 0, seatRow, seatCol);
  let nw = checkPath(seats, -1, -1, seatRow, seatCol);
  let paths = [n, ne, e, se, s, sw, w, nw];
  let occupiedSeats = paths.reduce((acc, path) => {
    return acc += (path) ? 1 : 0;
  }, 0);
  return occupiedSeats;
}

function doPass(seats, checkRoutine, comfortLevel) {
  let newState = [];
  seats.forEach((row, rowIndex) => {
    let newRow = '';
    row.split('').forEach((seat, colIndex) => {
      if (seat != '.') {
        let occupiedSeats = checkRoutine(seats, rowIndex, colIndex);
        if (phase) {
          ...
          if (seat == '#' && occupiedSeats >= comfortLevel) {
            newRow += 'L';
          } else {
            newRow += seat;
          }
...
function countSeats(state) {
  let emptySeats = 0;
  let occupiedSeats = 0;
  state.split('').forEach((seat) => {
    if (seat == 'L') {
      emptySeats++;
    }
    if (seat == '#') {
...
function findStasis(seats, checkRoutine, comfortLevel) {
  let currentState = seats.join('');
  let newSeats = doPass(seats, checkRoutine, comfortLevel);
  let newState = newSeats.join('');
  if (newState != currentState) {
    findStasis(newSeats, checkRoutine, comfortLevel);
  } else {
    log(countSeats(currentState));
  }
}

findStasis(rows, checkSurroundingSeats, 4);
findStasis(rows, checkSurroundinSeatsAdvanced, 5);