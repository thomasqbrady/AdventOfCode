...
input = `striped white bags contain 4 drab silver bags.
...
rules.map((rule) => {
  ...
  let parentBagColor = parts[0];
  if (!ruleMap[parentBagColor]) {
    ruleMap[parentBagColor] = {};
  }
  let children = parts[1].split(',');
  children.map((child) => {
    if (child.trim().toLowerCase() != 'no other bags.') {
      parts = child.trim().split(' ');
      parts.splice(parts.length - 1);
      let numBags = parseInt(parts[0], 10);
      let bagColor = parts.splice(1,).join(' ');
      ruleMap[parentBagColor][bagColor] = numBags;
    } else {
      ruleMap[parentBagColor] = 1;
    }
  });
});

let parentBags = Object.keys(ruleMap);

function findParentBags(searchColor, bags) {
  parentBags.map((parentColor) => {
    if (ruleMap[parentColor][searchColor]) {
      if (!bags[parentColor]) {
        bags[parentColor] = 1;
      } else {
        bags[parentColor]++;
      }
      findParentBags(parentColor, bags);
    }
  });
  return Object.keys(bags);
}

// console.log(findParentBags('shiny gold', {}).length);

let bagCount = 0;
let bagMap = {color: 'shiny gold', count: 1, children: []};

function countBags(level, multiplier = 1) {
  let bags = ruleMap[level.color];
  console.log(bags, level.color, level.count, multiplier);
  let bagColors = Object.keys(bags);
  bagColors.map((color) => {
    let child = {};
    child.color = color;
    child.count = bags[color];
    child.children = [];
    level.children.push(child);
    bagCount += multiplier * child.count;
    countBags(child, multiplier * child.count);
  });
}
countBags(bagMap);
