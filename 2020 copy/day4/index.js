...
function verifyHGT(passport) {
  let height = passport['hgt'];
  let measurement = height.substr(0, height.length - 2);
  let unit = height.substr(height.length - 2);
  switch (unit) {
    case 'cm':
...
function verifyHCL(passport) {
  ...
  if (hcl.length != 7) {
    ...
    if (hcl.substr(0,1) != '#') {
      ...
      let regex = RegExp('[0-9a-f]');
      let allValidCharacters = true;
      for (let i = 1;i < hcl.length;i++) {
        if (!regex.test(hcl.toLowerCase().substr(i))) {
          allValidCharacters = false;
        }
      }
      if (!allValidCharacters || hcl.length != 7) {
        passport.valid = false;
      }
    }
  }
...
function thoroughCheck() {
  ...
  documents.map((document) => {
    ...
    if (passport['byr'] && 
      ...
      passport = verifyBYR(passport);
      passport = verifyIYR(passport);
      passport = verifyEYR(passport);
      passport = verifyHGT(passport);
      passport = verifyHCL(passport);
      passport = verifyECL(passport);
      passport = verifyPID(passport);
