...
function getMinMax(crabPositions) {
  let min = 0, max = 0;
  crabPositions.forEach((position) => {
    min = Math.min(parseInt(position, 10), min);
    max = Math.max(parseInt(position, 10), max);
  });
  return {
    min: min,
    max: max
...
let minMax = getMinMax(crabPositions);
console.log({median, modes, minMax});

let minFuelUsed = 9999999999999999999999999999999999999999999999999999;
for (let position = minMax.min;position < minMax.max;position++) {
  let totalFuelUsed = 0;
  crabPositions.forEach((otherPosition) => {
    let fuelUsed = Math.abs(position - otherPosition);
    totalFuelUsed += (fuelUsed * (fuelUsed + 1)) / 2;
  });
  console.log({position, totalFuelUsed});
